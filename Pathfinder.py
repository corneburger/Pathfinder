...
pygame.display.set_caption("A* Path Finding Algorithm") # Window caption
...
lbl_distance = Label(option_menu, text='Distance Calculation: ')
r_euclidean = Radiobutton(option_menu, text = "Manhattan ", value = 1, var = distance_radio)
r_manhattan = Radiobutton(option_menu, text = "Euclidean ", value = 2, var = distance_radio)
lbl_algorithm = Label(option_menu, text='Algorithm: ')
r_A_star = Radiobutton(option_menu, text = "A* ", value = 1, var = algorithm_radio)
r_A_star_bounded = Radiobutton(option_menu, text = "A* with Bounded relaxation ", value = 2, var = algorithm_radio)
r_dijkstra = Radiobutton(option_menu, text = "Dijkstra's ", value = 3, var = algorithm_radio)
submit = Button(option_menu, text='Submit', command=onsubmit)

lbl_distance.grid(row = 2)
r_euclidean.grid(row = 3)
r_manhattan.grid(row = 4)
lbl_algorithm.grid(row = 5)
r_A_star.grid(row = 6)
r_A_star_bounded.grid(row = 7)
r_dijkstra.grid(row = 8)
submit.grid(row = 9)
option_menu.update()
mainloop()
...
class node: # Class to keep track of nodes (Cubes)
    ...
    def update_neighbors(self, grid):   # Check satus of neighbors to make sure not barriers
        ...
        if self.row > 0 and not grid[self.row][self.col - 1].is_barrier(): # Check if not left edge and check if row to left is not a barrier
            self.neighbors.append(grid[self.row][self.col - 1])
        ...
        for event in pygame.event.get():    # Loop through events that happend and check what they are
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE and start and end: # Use space to start algorithm and make sure we have a start and end node
                    ...
                    if (algorithm_radio.get() == 1):     # A-star     
