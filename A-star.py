...
def h(p1, p2):  #Distance measurement. Use manhattan/ absolute distance because easiest to compute
...
def A_star(draw, grid, start, end):
    ...
    # Current shortest distance from start node to current node (each node increases the g_score by one)
    ...
    # f_score  = g_score + h    (h is the predicted distance to node from current node to end node)
    ...
    f_score[start] = h(start.get_pos(), end.get_pos())  # Start has no g_score (Just approximation for distance (h_score))
        ...
        for event in pygame.event.get():    # Loop through events that happend and check what they are
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE and start and end: # Use space to start algorithm and make sure we have a start and end node
                    ...
                    A_star(lambda: draw(win, grid, ROWS, width), grid, start, end)    # Use lamda (Anonymous function to pass the function to use it directly)
                    stop_time = time.time()
